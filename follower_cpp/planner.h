...
class planner
{
    ...
    std::list<std::pair<int, int>> bad_cells;
    ...
    void update_h_values(std::pair<int, int> g)
    {
        ...
        while(!open.empty())
        {
            ...
            for(auto n: get_neighbors({current.i, current.j})) {
                float cost(1);
                if(use_precalc_cost)
                    cost = penalties[current.i][current.j];
...
public:
    planner(std::vector<std::vector<int>> _grid={}, float _use_precalc_cost=1.0, float _use_dynamic_cost=1.0, bool _reset_dynamic_cost=true):
    grid(_grid), use_precalc_cost(_use_precalc_cost), use_dynamic_cost(_use_dynamic_cost), reset_dynamic_cost(_reset_dynamic_cost)
    {
        abs_offset = {0, 0};
        goal = {0,0};
        start = {0, 0};
    ...
    void update_occupied_cells(const std::list<std::pair<int, int>>& _occupied_cells, std::pair<int, int> cur_goal)
    {
        ...
        for(auto o:_occupied_cells)
            num_occupations[o.first][o.second] += 1.0;//penalties[o.first][o.second];///_occupied_cells.size();
    }
    ...
    void update_bad_cells(const std::list<std::pair<int, int>>& _bad_cells)
    {
        for(auto b:bad_cells)
            grid[b.first][b.second] = 0;
        for(auto b:_bad_cells)
