...
void Follower::init(const Config& cfg_, py::array_t<double> array, std::vector<std::pair<int, int>> abs_starts)
{
    ...
    planners.clear();
    planner temp_planner(grid, cfg.use_precalc_cost, cfg.use_dynamic_cost, cfg.reset_dynamic_cost);
    ...
    for(size_t agent_idx = 0; agent_idx < agents.size(); agent_idx++)
    {
        ...
        planners.emplace_back(grid, cfg.use_precalc_cost, cfg.use_dynamic_cost, cfg.reset_dynamic_cost);
